...
async function workspace(): Promise<string> {
  return fixture({
    ...
    'apps/example/package.json': json({
      name: 'example',
      version: '4.0.0',
      private: true,
    }),
...
describe('workspaceDirs', () => {
  ...
  test('empty for a single-package repository', async () => {
    const root = await fixture({
      'package.json': json({ name: 'solo', version: '1.0.0' }),
    })
  ...
  test('never walks into node_modules', async () => {
    const root = await fixture({
      ...
      'packages/a/node_modules/vendored/package.json': json({
        name: 'vendored',
        version: '9.9.9',
      }),
    })
...
describe('setVersion', () => {
  test('bumps the publishable packages and the root, leaves private ones', async () => {
    ...
    const changes = await jsAdapter.setVersion({
      root,
      version: '1.3.0',
      dryRun: false,
    })
    ...
    expect(
      JSON.parse(await Bun.file(`${root}/packages/core/package.json`).text())
        .version,
    ).toBe('1.3.0')
    ...
    expect(
      JSON.parse(await Bun.file(`${root}/apps/example/package.json`).text())
        .version,
    ).toBe('4.0.0')
    expect(
      changes.find(c => c.file === 'apps/example/package.json')?.state,
    ).toBe('unchanged')
  })
